...
export function PartnerInvitationEmail({
  ...
  const previewText = `You're invited to join ${partnerName}`
    ...
    <Html>
      ...
      <Body style={main}>
        <Container style={container}>
          ...
          <Heading style={h1}>You're invited to join {partnerName}</Heading>

          <Text style={text}>
            {inviterName
              ? `${inviterName} has invited you to join ${partnerName} as a ${roleLabel}.`
              : `You've been invited to join ${partnerName} as a ${roleLabel}.`}
          </Text>
          ...
          {workspaceAssignments && workspaceAssignments.length > 0 && (
            <Section style={workspacesSection}>
              <Text style={workspacesTitle}>You'll have access to these workspaces:</Text>
              {workspaceAssignments.map((ws, idx) => (
                <Text key={idx} style={workspaceItem}>
                  • {ws.name} ({ws.role})
                </Text>
              ))}
            </Section>
