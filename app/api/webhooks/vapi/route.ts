...
export async function POST(request: NextRequest) {
  try {
    ...
    if (signatureVerification.skipped) {
      console.log("[VAPI Webhook] Signature verification skipped (no VAPI_WEBHOOK_SECRET configured)")
    }
 ...
 */
function extractTranscriptMessages(call: VapiCall, payload?: VapiWebhookPayload): VapiTranscriptMessage[] {
  // Priority 1: ROOT LEVEL messages (VAPI's ACTUAL format!)
  if (payload?.messages && payload.messages.length > 0) {
    return payload.messages
  }
...
async function handleStatusUpdate(payload: VapiWebhookPayload) {
  ...
  if (!conversation && assistantId) {
    ...
    if (agent?.workspace) {
      ...
      const validStartedAt = parsedStartedAt && parsedStartedAt.getTime() > 0 && parsedStartedAt.getFullYear() > 2000 
        ? parsedStartedAt 
        : new Date()
  ...
  if (conversation) {
    let dbStatus: "initiated" | "ringing" | "in_progress" | "completed" | "failed" = conversation.status as "initiated" | "ringing" | "in_progress" | "completed" | "failed"
...
async function handleEndOfCallReport(payload: VapiWebhookPayload) {
  ...
  if (!conversation && assistantId) {
    ...
    const validStartedAt = startedAt && startedAt.getTime() > 0 && startedAt.getFullYear() > 2000 ? startedAt : new Date()
    const validEndedAt = endedAt && endedAt.getTime() > 0 && endedAt.getFullYear() > 2000 ? endedAt : new Date()
    ...
    conversation = await prisma.conversation.create({
      data: {
        ...
        metadata: {
          ...
          emotions: call.analysis?.emotions ? JSON.parse(JSON.stringify(call.analysis.emotions)) : undefined,
        },
      },
      include: {
        workspace: { select: { id: true, partnerId: true } },
    ...
    // Validate timestamps are reasonable (not Unix epoch / year 1970)
    const startedAt = parsedStartedAt && parsedStartedAt.getTime() > 0 && parsedStartedAt.getFullYear() > 2000 
      ? parsedStartedAt 
      : conversation.startedAt
    const endedAt = parsedEndedAt && parsedEndedAt.getTime() > 0 && parsedEndedAt.getFullYear() > 2000 
      ? parsedEndedAt 
      : new Date()
    ...
    // Summary can be at root level or in analysis
    const summary = payload.summary || payload.analysis?.summary || call.analysis?.summary || conversation.summary
  ...
  const validBillingStartedAt = billingStartedAt && billingStartedAt.getTime() > 0 && billingStartedAt.getFullYear() > 2000 
    ? billingStartedAt 
    : conversation.startedAt
  const validBillingEndedAt = billingEndedAt && billingEndedAt.getTime() > 0 && billingEndedAt.getFullYear() > 2000 
    ? billingEndedAt 
    : new Date()
  
  const billingDurationMs = validBillingStartedAt && validBillingEndedAt 
  ...
  if (finalConversation && finalConversation.agent) {
    try {
      ...
      console.log(`[VAPI Webhook] Indexing call ${call.id} to Algolia with cost: ${finalConversation.totalCost}`)
      ...
      if (indexResult.success) {
        ...
        console.warn(`[VAPI Webhook] Algolia indexing skipped for call ${call.id}: ${indexResult.reason}`)
      }
    } catch (err) {
